...
def beam_search_t(sentence, cr, deploy_model, answer_dict, minLen=1, maxLen=50, search_scope=200,
    ...
    def step(last_sentence, score, pred_words_prob_array_old,
             pred_word_array_old, beam_small_value, beam_full_flag):
        ...
        for i in range(min(len(pred_words_prob_array), 10)):
            ...
            if available_flag < current_score / (len(last_sentence) + 1):
                if DEBUG:
                    print("current_score / (len(last_sentence) + 1):", current_score / (len(last_sentence) + 1))
                pass
                continue
            new_sentence = last_sentence + [pred_word]
            if DEBUG:
            ...
            if pred_word == stop_tag:
                if len(new_sentence) < minLen:
                    print("sentence too short:"), new_sentence
                    continue
                #cands.append(SentenceScorePair(score - math.log(pred_word_prob), new_sentence))
                if DEBUG:
                    print "cands score:", (score - math.log(pred_word_prob)) / len(new_sentence)
                    print "cands sent:", new_sentence
                ...
                if(beam_full_flag and beam_small_value < score - math.log(pred_word_prob)):
                    print "beam_small_value < score - math.log(pred_word_prob):", score - math.log(pred_word_prob)
                    continue
                if DEBUG:
                    print "results score:", score - math.log(pred_word_prob)
                    print "results sent:", new_sentence
