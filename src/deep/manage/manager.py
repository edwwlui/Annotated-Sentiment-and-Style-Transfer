...
class ModelManager:
    ...
    def generate(self, input_file, output_file):
        ...
        with open(output_file, 'w') as fw:
            with codecs.open(input_file, 'r', config.globalCharSet(), errors='replace') as fo:
    ...
    def generate_b_v(self, input_file, output_file):
        ...
        """
        deploy_model = self.model.get_deploy_function()
        with codecs.open(output_file, 'w', config.globalCharSet(), errors='replace') as fw:
            with codecs.open(input_file, 'r', config.globalCharSet(), errors='replace') as fo:
                for line in fo.readlines():
                    # line_word, line_zi = SegProcess(line.strip())
    ...
    def generate_b_v_t(self, input_file, output_file):
        ...
        deploy_model = self.model.get_deploy_function()
        print("manager output_file:", output_file)
        start = time.clock()
        with codecs.open(output_file, 'w', config.globalCharSet(), errors='replace') as fw:
            with codecs.open(input_file, 'r', config.globalCharSet(), errors='replace') as fo:
                for line in fo.readlines():
                    ...
                    # print score
                    if(len(res) <= 0):
                        print 'manager.py not find:', line1
                        fw.write(line + '\t' + 'None' + '\t' + str(0) + '\n')
                        continue
                    for i in range(1):
                        res1 = [res[i][1:-1]]
                    # print res,score
        ...
        print "manager.py read: %f s" % (end - start)
