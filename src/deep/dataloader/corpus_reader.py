...
class CorpusReader:
    ...
    def __init__(self, dataset_file=None, stopwords_file=None,
        ...
        # load stop words
        print('src/deep/dataloader/corpus_reader.py: Reading stop words')
        self.stopwords = load_stopwords_file(stopwords_file, charset)
        print('src/deep/dataloader/corpus_reader.py: Number of stop words: %i' % len(self.stopwords))
        # load word2index
        print('src/deep/dataloader/corpus_reader.py: Reading word2index')
        if dict_file is None:
            raise Exception('src/deep/dataloader/corpus_reader.py: Word2index should exist')
        else:
            self.word2index, self.index2word = load_dictionary_file(dict_file,
                                                                    special_flag=self.special_flag,
                                                                    charset=charset)
            print('src/deep/dataloader/corpus_reader.py: Dictionary size: %i' % len(self.word2index))
        # load word embedding
        if word_embedding_file is not None:
            print('src/deep/dataloader/corpus_reader.py: Reading word embedding.')
            self.word_embedding_dict = load_word_embeddings_file(word_embedding_file, charset)
            print('src/deep/dataloader/corpus_reader.py: Merging word embedding')
        ...
        print('src/deep/dataloader/corpus_reader.py: Writing word2index used')
        ...
        if dataset_file is not None:
            print('src/deep/dataloader/corpus_reader.py: loading data')
            dataset = self.load_data(dataset_file, charset)
            print('src/deep/dataloader/corpus_reader.py: dividing data set')
            if(len(dataset[0]) == 2):
            ...
            print('src/deep/dataloader/corpus_reader.py: Train_set size: %i' % len(self.train_set[0]))
            print('src/deep/dataloader/corpus_reader.py: Valid_set size: %i' % len(self.valid_set[0]))
            print('src/deep/dataloader/corpus_reader.py: Test_set size: %i' % len(self.test_set[0]))

        print('src/deep/dataloader/corpus_reader.py: Finish reading data')
